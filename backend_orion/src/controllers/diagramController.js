...
export class DiagramController {
    ...
    async generateDiagrams(req, res) {
        try {
            ...
            // ═══════════════════════════════════════════════════
            // 5a. Mermaid via IA
            const mermaidCode = await this.diagramService.generateArchitectureDiagram(project.name, project.type, filesContent);
            // 5b. D2 via IA
            const d2Code = await this.diagramService.generateD2Diagram(project.name, project.type, filesContent);
            // 5c. Isoflow JSON via IA
            const isoflowModel = await this.diagramService.generateIsoflowModel(project.name, project.type, filesContent);
            // ═══════════════════════════════════════════════════
            ...
            // ═══════════════════════════════════════════════════
            await this.saveArtifact(analysis.id, 'architecture_mermaid.md', 'mermaid', mermaidCode, 'Diagrama Mermaid de arquitectura generado por IA');
            await this.saveArtifact(analysis.id, 'architecture_d2.d2', 'd2', d2Code, 'Diagrama D2 de arquitectura generado por IA');
            await this.saveArtifact(analysis.id, 'architecture_isoflow.json', 'isoflow', JSON.stringify(isoflowModel, null, 2), 'Modelo isométrico de arquitectura generado por IA');
            // 8. Historial
     ...
     */
    async getProjectDiagrams(req, res) {
        try {
            const projectId = req.params.projectId;
     ...
     */
    async getIsometricImage(req, res) {
        try {
            const projectId = req.params.projectId;
            if (!projectId) {
     ...
     */
    async getIsometricHTML(req, res) {
        try {
            const projectId = req.params.projectId;
            if (!projectId) {
                return res.status(400).json({ success: false, error: 'Proyecto no especificado' });
     ...
     */
    async getDiagramImage(req, res) {
        try {
            const projectId = req.params.projectId;
            const format = req.params.format; // 'mermaid' | 'd2'
            if (!projectId || !format) {
     ...
     */
    async getProjectFiles(projectId, analysis) {
        const files = {};
        ...
        if (codeIndex?.indexData) {
            try {
                const indexData = codeIndex.indexData;
     ...
     */
    async saveArtifact(analysisId, name, format, content, description) {
