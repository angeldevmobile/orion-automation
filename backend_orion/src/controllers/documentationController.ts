...
import { PrismaClient, type Project, type ProjectSource } from '@prisma/client';
...
export class DocumentationController {
  ...
  async downloadDocument(req: Request, res: Response) {
    try {
      ...
      if (format === 'pdf') {
        ...
        res.setHeader(
          'Content-Disposition',
          `attachment; filename="${artifact.name.replace('.md', '.pdf')}"`
        );
  ...
  private async generateDocContentWithAI(
