...
IMPORTANTE: Responde SOLO en español. No incluyas texto ni recomendaciones en inglés.`;
     ...
     */
    async analyzeDimension(index, dimension) {
        const prompts = {
...
- Responde SOLO en español`;
    ...
    buildSecurityPrompt(index) {
        return `Evalúa seguridad con estas señales:

**RIESGOS DETECTADOS:**
${index.riskSignals.filter(s => s.includes('password') ||
            s.includes('token') ||
            s.includes('secret') ||
            s.includes('hardcoded')).join('\n')}
...
**SEÑALES:**
${index.riskSignals.filter(s => s.includes('Sequential') ||
            s.includes('await') ||
            s.includes('for')).slice(0, 5).join('\n')}
...
**SEÑALES:**
${index.riskSignals.filter(s => s.includes('long file') ||
            s.includes('Empty catch') ||
            s.includes('TODO')).slice(0, 5).join('\n')}
...
- Responde SOLO en español`;
    ...
    parseStructuralResponse(content) {
        try {
    ...
    parseDimensionResponse(content, dimension) {
        try {
            const jsonMatch = content.match(/```json\s*([\s\S]*?)\s*```/) || content.match(/\{[\s\S]*\}/);
            const jsonText = jsonMatch ? (jsonMatch[1] || jsonMatch[0]) : content;
            const parsed = JSON.parse(jsonText);
