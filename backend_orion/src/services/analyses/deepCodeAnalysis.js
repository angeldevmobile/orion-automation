...
export class DeepCodeAnalysisService {
    ...
    async analyzeFile(filePath) {
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            const prompt = this.buildDeepPrompt(filePath, chunk.content, chunk.startLine, i + 1, chunks.length);
    ...
    chunkContentByLines(content, maxLinesPerChunk) {
    ...
    buildDeepPrompt(filePath, content, startLine, chunkIndex, totalChunks) {
...
- Responde SOLO en español. No incluyas texto en inglés.`;
    ...
    parseDeepResponse(response, startLine) {
        try {
            ...
            return {
                ...
                issues: Array.isArray(parsed.issues)
                    ? parsed.issues
                        .filter((i) => i && typeof i === 'object' && 'description' in i)
                        .map((issue) => ({
                        ...issue,
...
//# sourceMappingURL=deepCodeAnalysis.js.map