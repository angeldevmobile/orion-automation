...
export class DecisionController {
    ...
    async getAnalysisDecisions(req, res) {
        try {
            ...
            for (const analysis of analyses) {
                if (analysis.result) {
                    try {
                        ...
                        const decision = decisions.find((d, index) => (d.id === decisionId || `${analysis.id}-${index}` === decisionId));
                        if (decision) {
                            foundDecision = {
                                ...decision,
                                metadata: {
                ...
                if (analysis.result) {
                    try {
                        const parsedResult = typeof analysis.result === 'string'
                            ...
                            : analysis.result;
                        const decisions = parsedResult.decisions || [];
                        const decisionIndex = decisions.findIndex((d, index) => (d.id === decisionId || `${analysis.id}-${index}` === decisionId));
                        if (decisionIndex !== -1) {
